...
SELECT party, COUNT(*)
FROM (SELECT party, RANK() OVER(PARTITION BY constituency ORDER BY votes DESC) AS r 
      FROM ge 
      WHERE yr = '2017' 
      AND constituency LIKE 'S%') AS rr
WHERE r= '1'
GROUP BY party;


SELECT party, SUM(win)
FROM (
      SELECT party, 
             (CASE WHEN 
                1 = (RANK() OVER (PARTITION BY constituency ORDER BY votes DESC)) THEN 1
              ELSE 0
              END) AS win
      FROM ge
...
-- Show how many votes for each party who won seats in Scotland in 2017 . 
SELECT party, SUM(win)
FROM (
      SELECT party, 
             (CASE WHEN 
                1 = (RANK() OVER (PARTITION BY constituency ORDER BY votes DESC)) THEN votes
              ELSE 0
              END) AS win
      FROM ge
      WHERE constituency LIKE 'S%'
